...
def infer_node_classifier(test_data_splits_dir, test_data_dir, density_map_name, atom_checkpoint, infer_run_on, infer_on_gpu):
    print(f"Running ATOM prediction for ==> {density_map_name}")
    print(f"Running ATOM prediction using check point ==> {atom_checkpoint}")
    ...
    print(f"ATOM MRC file prediction completed and saved in ==> {outfilename}")

    # save the probabilities
    file_prob = f"{test_data_dir}/{density_map_name}/{density_map_name}_probabilities_atom.txt"
    save_probs(outfilename, idx_val_mat, file_prob)
    print(f"ATOM PROBABILITIES file generation completed and saved in ==> {file_prob}")
